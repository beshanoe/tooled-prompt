...
export class TooledPromptEmitter {
   ...
   */
  on<K extends keyof TooledPromptEvents>(
    event: K,
   ...
   */
  off<K extends keyof TooledPromptEvents>(
    event: K,
    handler: EventHandler<K>,
  ): void {
   ...
   */
  emit<K extends keyof TooledPromptEvents>(
    event: K,
    ...args: Parameters<TooledPromptEvents[K]>
  ): void {
 ...
 */
export function defaultToolCallHandler(
  name: string,
  ...
  for (const [key, value] of Object.entries(args)) {
    if (typeof value === "string" && value.length > 100) {
      argParts.push(
        `  ${key}: "${value.slice(0, 100)}..." (${value.length} chars)`,
      );
 ...
 */
export function defaultToolResultHandler(
  _name: string,
  result: string,
  _duration: number,
): void {
  const display =
    result.length > 200
      ? result.slice(0, 200) + `... (${result.length} total chars)`
      : result;
  ...
  target.on("content", (content) => {
    if (!hasContent) {
      ...
      if (!silent()) {
        ...
        writeStdout(
          prefix + formatLabel("[Response]", "response") + " ",
        );
      }
      inThinking = false;
    }
